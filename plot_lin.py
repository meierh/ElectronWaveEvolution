...
def plot_csv(filename):
    ...
    # Set both axes to linear scale (this is the default, so we don't need to set it explicitly)
    # Removed: plt.xscale('log') and plt.yscale('log')
    ...
    plt.title(f'Linear-Scale Plot of {filename}')  # Set the title using the filename
    ...
    output_filename = filename.split('.')[0] + '_linear_plot_with_numbered_points_in_legend.png'
    plt.savefig(output_filename)
    print(f'Linear-Scale Plot with numbered points in the legend saved as {output_filename}')
    plt.close()

if __name__ == "__main__":
    # Set up command-line argument parsing
    parser = argparse.ArgumentParser(description='Plot and save data from a CSV file with linear scales and numbered points in legend.')
